...
/*utilizadores temporários*/
const utilizadoresTemporarios = [
  { email: "Jorge", password: "12345" },
  { email: "Tomás", password: "67890" }
]
...
endpointsFunction.login = async (req, res) => {
  const { email, password } = req.body;
  try {
    let allowLoggin = null;
    for (utilizador of utilizadoresTemporarios) {
      if (email == utilizador.email && password == utilizador.password) {
        allowLoggin = utilizador;
        break;
      }
    }
    if (!allowLoggin) {
    ...
    const token = jwt.sign({ email: allowLoggin.email }, config.secret, {
      expiresIn: config.timer,
    });
    res.status(200).json({
      success: true,
